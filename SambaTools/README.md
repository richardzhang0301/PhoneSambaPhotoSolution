...
`run.bat` creates a local `.venv` when needed, uses it on later runs, and can install Pillow into that environment if it is missing. Video thumbnails require `ffmpeg` on `PATH` and include a semi-transparent play overlay.
